...
export const currentFilterProfileMissingFields = computed(() => {
  ...
  currentWebsiteFilterNames.value.forEach((field) => {
...
export const currentWebsiteMissingFields = computed(() => {
  ...
  return profileFieldNames.filter((field) => !filterFieldNames.includes(field))
})
