A Chrome Extension Built in Vue, with the purpose of making people's job search slightly less tedious by enhancing the standard filtering options of job sites.
...
* Correctly apply filter from the moment the page is opened
    issue is that filter runs before storage.get is run.
    so it is running the filter method with empty data, hence the filter doesn't work
